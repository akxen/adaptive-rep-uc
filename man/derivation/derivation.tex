...
\newcommand{\cScenarioProbibility}[1][\iScenario]{\pi_{#1}}
...
\begin{equation}
	\cSchemeRevenue[\iCalibrationInterval] + \sum\limits_{\iCalibrationIntervalAlias=\iCalibrationInterval}^{\iCalibrationInterval + \cTotalCalibrationIntervals} \sum\limits_{\iScenario \in \sScenarios} \cScenarioProbibility \sum\limits_{\iGenerator \in \sGenerators} \left(\cCalibrationIntervalEmissions - \vBaseline\right) \cCalibrationIntervalEnergy \vPermitPrice = \cSchemeRevenue[\iCalibrationInterval+\cTotalCalibrationIntervals]
	\label{eqn: revenue target}
\end{equation}

\begin{equation}
	\cSchemeRevenue[\iCalibrationInterval] + \sum\limits_{\iCalibrationIntervalAlias=\iCalibrationInterval}^{\iCalibrationInterval + i} \sum\limits_{\iScenario \in \sScenarios} \cScenarioProbibility \sum\limits_{\iGenerator \in \sGenerators} \left(\cCalibrationIntervalEmissions - \vBaseline\right) \cCalibrationIntervalEnergy \vPermitPrice \geq \cSchemeRevenueLowerBound \quad \forall i \in \left[1, 2, \ldots, \cTotalCalibrationIntervals \right]
